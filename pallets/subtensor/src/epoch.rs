...
pub trait SubtensorEpochTypeTrait<T> {
    fn to_subtensor_epoch_result(&self) -> Vec<I32F32>;
}
impl<T: Config> SubtensorEpochTypeTrait<T> for Vec<(T::AccountId, u64, u64)> {
    fn to_subtensor_epoch_result(&self) -> Vec<I32F32> {
        self.encode().iter().map(|&x| I32F32::from_num(x)).collect()
    }
}
impl<T: Config> SubtensorEpochTypeTrait<T> for Vec<I32F32> {
    fn to_subtensor_epoch_result(&self) -> Vec<I32F32> {
        return self.clone();
    }
}
...
impl<T: Config> Pallet<T> {
    ...
    #[allow(clippy::indexing_slicing)]
    pub fn subtensor_epoch(netuid: u16, is_incentive: Option<bool>) ->  Box<dyn SubtensorEpochTypeTrait<T>>  {
        ...
        if is_incentive {
            // Insentive I32F32 as SubtensorEpochTypeTrait
            Box::new(incentive) as Box<dyn SubtensorEpochTypeTrait<T>>
        } else {
            // Emission tuples ( hotkeys, server_emission, validator_emission ) as SubtensorEpochTypeTrait
            Box::new(hotkeys
                .into_iter()
                .map(|(uid_i, hotkey)| {
                    (
                ...
                .collect::<Vec<_>>()) as Box<dyn SubtensorEpochTypeTrait<T>>
        }
    }
